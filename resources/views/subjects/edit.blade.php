...
@section('page-title', 'Modifier la Matiere')
@section('page-subtitle', $subject->name_fr)

@section('breadcrumb')
    <a href="{{ route('subjects.index') }}" class="hover:text-gray-700">Matieres</a>
    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="font-medium" style="color: #1A3A6B;">{{ $subject->name_fr }}</span>
@endsection

@section('content')
@php
    $currentSectionId = $subject->category?->section_id;
...
<div class="w-full px-4 lg:px-8" x-data="{
    sectionId: '{{ old('section_id', $currentSectionId) }}',
    ...
    <form method="POST" action="{{ route('subjects.update', $subject) }}">
        ...
        <div class="max-w-4xl rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8">
            <div class="mb-7 border-b border-gray-100 pb-4">
                ...
                <p class="mt-1 text-sm text-gray-500">La categorie proposee depend de la section choisie.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $subject->name_fr) }}" maxlength="100" class="w-full rounded-xl border px-4 py-3 text-sm outline-none focus:border-[#1A3A6B] focus:ring-2 focus:ring-blue-100 {{ $errors->has('name') ? 'border-red-400' : 'border-gray-200' }}">
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-[#0F4670] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#0C395A]">
