...
<head>
    <meta charset="UTF-8">
    <title>Bulletin de paie — {{ $staff->full_name }}</title>
    <style>
        @page { size: A4 portrait; margin: 4mm; }
        body { font-family: Arial, Helvetica, sans-serif; margin: 0; color: #000; background: #fff; padding: 0; }
        ...
        .sheet { padding: 8px 16px 16px; }
        .sheet + .sheet { margin-top: 12px; page-break-inside: avoid; }
        .bordereau-header { display: grid; grid-template-columns: 1.3fr 1fr; gap: 18px; align-items: start; padding-bottom: 12px; margin-bottom: 10px; border-bottom: 1px solid #CBD5E1; color: #1A3A6B; }
        ...
        .bordereau-header__title-row { text-align: center; margin-bottom: 12px; }
        .bordereau-header__title { font-size: 26px; font-weight: 900; text-transform: uppercase; text-decoration: underline; color: #1A3A6B; }
        .bordereau-header__subtitle { font-size: 13px; margin-top: 4px; color: #1A3A6B; }
        .table { width: 100%; border-collapse: collapse; font-size: 13px; }
        .table th, .table td { border: 1px solid #000; font-size: 14px; padding: 8px; vertical-align: top; }
        .table th { text-align: left; font-weight: 400; background: #fff; }
        .table td { font-weight: 700; color: #000; }
        .totals { margin-top: 12px; padding: 12px 14px; border: 1px solid #000; display: flex; justify-content: space-between; align-items: center; gap: 24px; }
        .totals__item { display: flex; align-items: center; width: 48%; font-size: 13px; line-height: 1.4; }
        .totals__label { font-weight: 400; margin-right: 3rem; white-space: nowrap; }
        .totals__value { font-weight: 700; color: #000; font-size: 15px; }
        .signature-row { margin-top: 18px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
...
<div class="sheet">
    ...
    <div class="bordereau-header__title-row">
        <div class="bordereau-header__title">BULLETIN DE PAIE</div>
        {{-- <div class="bordereau-header__subtitle">Document papier de paie</div> --}}
    </div>
    ...
    <div class="totals">
        ...
        <div class="totals__item" style="justify-content: flex-end;">
            ...
            <span class="totals__value">{{ $amountReceived ? number_format((float) $amountReceived, 0, ',', ' ') . ' FCFA' : 'À renseigner' }}</span>
        </div>
    </div>
...
<div class="sheet" style="margin-top: 5rem;">
    <div class="bordereau-header">
        <div class="bordereau-header__brand">
            <div class="bordereau-header__logo">
                @if($logoSrc)
